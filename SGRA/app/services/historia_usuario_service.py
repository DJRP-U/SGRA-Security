...
class HistoriaUsuarioService:
    ...
    def listar_por_proyecto(self, proyecto_uuid: str, skip: int, limit: int, solicitante: Cuenta, prioridad: Optional[Prioridad] = None, estado: Optional[Estado] = None):
        proyecto = (
        ...
        # Base de la consulta con join para asegurar que pertenecen al proyecto
        base_query = (
            self.db.query(HistoriaUsuario)
            .join(HistoriaUsuario.requisitos)
            .filter(
                Requisito.proyecto_id == proyecto.id,
                HistoriaUsuario.sprint_id.is_(None),  # solo HU sin sprint asignado
            )
        )
        ...
        if estado:
            ...
            # Por defecto ocultamos las obsoletas si no se filtra específicamente por ese estado
            base_query = base_query.filter(HistoriaUsuario.estado != Estado.OBSOLETA)

        # Conteo y obtención de resultados con distinct para evitar duplicados por el join
